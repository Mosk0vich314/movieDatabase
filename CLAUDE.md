...
- **Chart** (`#chart`): Two sub-tabs (`#chart-tabs`, `.chart-tab`) — **Ranked** (`#chart-list`: the score-ordered top 30, Movie Tournament, King of the Hill, external top-list links) and **My Top 10** (`#chart-top10`: the hand-ranked list). The tab choice is remembered in `localStorage.chartTab`. Flipping tabs never re-renders the ranked pane while a tournament or KOTH run is in progress.
- **My Top 10** (Chart sub-tab): A hand-ordered list of up to 10 films, because two films rated 9 don't settle which is better. Stored in `localStorage.manualTop10` as an array of movie ids, #1 first, and self-heals when a film is deleted. Ten slots; tapping an empty one opens a searchable catalogue picker (`.t10-picker`), filled rows have ▲ ▼ ✕ and open the detail page on tap. "Fill from ratings" seeds it with the top 10 by score as a starting point; "Make the poster" calls `Posters.openBoard()`.
